...
# Copyright (C) 2024 Philipp Allgeuer
...
# TODO: https://huggingface.co/openai/whisper-large-v3 (Transformers library acceleration)
# TODO: https://github.com/guillaumekln/faster-whisper (CTranslate2 library acceleration, integrates Silero VAD)
# TODO: https://github.com/snakers4/silero-vad
...
class SpeechASR:
	...
	def __init__(
		...
		#self.capture_queue.put('RequestItem')
		self.capture_queue: queue.Queue['RequestItem'] = queue.Queue()
		self.capture_condition = threading.Condition(lock=self.lock)

		self.infer_thread = None
		#self.infer_queue = queue.Queue()
		#self.infer_queue.put('InferRequestItem')
		self.infer_queue: queue.Queue['InferRequestItem'] = queue.Queue()
...
class WhisperASR(ASRModel):
	...
	def __init__(self, variant='small', audio_lang='english', translate=True, preload_model=True, use_cuda=True, verbose=False):
		...
		if use_cuda and not torch.cuda.is_available():
			rospy.logwarn("Whisper requested CUDA inference, but torch.cuda.is_available() is False. Falling back to CPU.")
			use_cuda = False
...
class Microphone:
	...
	def start_listening(self):
		...
		if self.queued:
			...
			self.queue: queue.Queue['byte'] = queue.Queue()
