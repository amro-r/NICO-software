#!/usr/bin/env python3
"""
Left arm range test (shoulder, elbow, wrist; fingers excluded)
...
- Excludes finger motors; finger range is covered by l_fingers_test.py.

This script is designed to test the newly replaced left arm hardware.
"""
...
# Left arm joint specifications (mirrored from right arm)
JOINT_SPECS = {
    "l_shoulder_y": {"description": "Shoulder Pitch", "desired_span": (-120, 120)},
    "l_shoulder_z": {"description": "Shoulder Roll", "desired_span": (-90, 90)},
    "l_arm_x":      {"description": "Upper Arm Twist", "desired_span": (-140, 5)},  # matches config default (note: inverted from right arm)
    "l_elbow_y":    {"description": "Elbow Flex", "desired_span": (-100, 100)},
    "l_wrist_z":    {"description": "Wrist Roll (left/right)", "desired_span": (-90, 90)},
    "l_wrist_x":    {"description": "Wrist Pitch (up/down)", "desired_span": (-35, 50)},
}
...
def load_joint_specs():
    ...
    # Hard fallback (left arm common IDs based on config)
    return {
        2: {"name": "l_shoulder_y", "description": "Shoulder Pitch", "desired_span": (-120, 120), "angle_limit": (-179, 180), "config": "hardcoded"},
        22: {"name": "l_shoulder_z", "description": "Shoulder Roll", "desired_span": (-90, 90), "angle_limit": (-125, 100), "config": "hardcoded"},
        4: {"name": "l_arm_x", "description": "Upper Arm Twist", "desired_span": (-140, 5), "angle_limit": (-140, 5), "config": "hardcoded"},
        6: {"name": "l_elbow_y", "description": "Elbow Flex", "desired_span": (-100, 100), "angle_limit": (-100, 100), "config": "hardcoded"},
        24: {"name": "l_wrist_z", "description": "Wrist Roll (left/right)", "desired_span": (-90, 90), "angle_limit": (-90, 90), "config": "hardcoded"},
        26: {"name": "l_wrist_x", "description": "Wrist Pitch (up/down)", "desired_span": (-35, 50), "angle_limit": (-35, 50), "config": "hardcoded"},
    }
...
def test_arm(port="/dev/ttyUSB0", baud=1_000_000):
    print("=" * 60)
    print("LEFT ARM RANGE TEST (Shoulder / Elbow / Wrist)")
    print("=" * 60)
    ...
    for result in results_summary:
        ...
        if result["span"] is not None:
            print(f"{status_icon} {result['name']} (ID {result['id']}): {result['status']} - span {result['span']:.1f}° [{result['min']:.1f}°, {result['max']:.1f}°]")
        else:
            print(f"{status_icon} {result['name']} (ID {result['id']}): {result['status']}")
    
    ok_count = sum(1 for r in results_summary if r["status"] == "OK")
    total_count = len(results_summary)
    print(f"\nTotal: {ok_count}/{total_count} joints passed")
